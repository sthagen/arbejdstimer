SHELL = /bin/bash
package = shagen/arbejdstimer

.DEFAULT_GOAL := all
isort = isort arbejdstimer tests
black = black -S -l 120 --target-version py38 arbejdstimer tests
...
.PHONY: test
test: clean
	pytest --cov=arbejdstimer --log-format="%(levelname)s %(message)s"
